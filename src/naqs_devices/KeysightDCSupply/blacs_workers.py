...
class KeysightDCSupplyWorker(VISAWorker):
    ...
    # E364
    esr_mask = 32 + 16 + 8 + 4 # = 60: Command, execution, device error, query error
    # esr_mask = 60
    # qsr_mask = 1539

    # qsr_mask = 1 + 2 + 16 + 32  # = 51
    qsr_mask = 1 + 2 + 16 + 32 + 64 + 512  # = 625, # Voltage/current/temp/power/config/comm

    # define the initialisation string
    init_string = f'*ESE {esr_mask};STAT:QUES:ENAB {qsr_mask};*CLS'
    # ident_string = 'E364'
    supported = ['E364', 'E36311A']
    ...
    def check_status(self):
        ...
        # response = self.connection.query_ascii_values('*ESR?;STAT:QUES?;QUES:COND?',separator=';') #364
        # response = self.connection.query_ascii_values('*ESR?;STAT:QUES?;STAT:OPER:COND?',separator=';')
        esr = int(self.connection.query('*ESR?'))
        qsr = int(self.connection.query('STAT:QUES?'))
        cond = int(self.connection.query('STAT:OPER:COND?'))
        self.logger.info([esr, qsr, cond])
